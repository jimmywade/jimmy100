...
$idVacante=  $_GET['idiVacante'];
    ...
    class general{
        ...
        public function obtener($Mpresa,$idVacante){
            ...
            $Mpresa=$Mpresa;
            $idVacante=$idVacante;
            $sqlb = " SELECT * FROM solicitudes AS so INNER JOIN vacantes AS va ON so.idVacante=va.idVacante WHERE so.idEmpresa='$Mpresa' AND so.estadoEmpresa=1 AND so.estadoPersona=1 AND va.idVacante='$idVacante' ";
